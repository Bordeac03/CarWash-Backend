...
public class WebConfig extends CoreJDBCDao implements WebMvcConfigurer {
    ...
    public void init() {
        ...
        String createCarWashServiceSQL = "CREATE TABLE IF NOT EXISTS carWashService (ID INT NOT NULL AUTO_INCREMENT, name varchar(255) NOT NULL, carWashID INT NOT NULL, price DOUBLE(11,2) NOT NULL, active BOOLEAN DEFAULT 1, PRIMARY KEY (ID), FOREIGN KEY (carWashID) REFERENCES carWash(ID), UNIQUE(`name`,`carWashID`));";
        String createBookingsSQL = "CREATE TABLE IF NOT EXISTS booking (ID INT NOT NULL AUTO_INCREMENT, userID INT NOT NULL, carWashID INT NOT NULL, serviceID INT NOT NULL, ts TIMESTAMP DEFAULT CURRENT_TIMESTAMP, closeBy BOOLEAN DEFAULT 0, active BOOLEAN DEFAULT 1 , PRIMARY KEY (ID), FOREIGN KEY (userID) REFERENCES user(ID), FOREIGN KEY (carWashID) REFERENCES carWash(ID), FOREIGN KEY (serviceID) REFERENCES carWashService(ID));";
